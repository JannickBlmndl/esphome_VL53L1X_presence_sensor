...
void VL53L1XSensor::update() {
    ...
    if (VL53L1X_check_for_data_ready() == 0) {
		uint16_t distance = VL53L1X_get_distance(); //Get the result of the measurement from the sensor
...
void VL53L1XSensor::loop() {
    ...
    if (check_for_data_ready() == 0) {
        ...
        ESP_LOGW(TAG, "'%s' - There's an error data not ready", this->name_.c_str();
...
bool VL53L1XSensor::VL53L1X_begin()
{
	...
	{
		status = this->write_byte(Addr, VL51L1X_DEFAULT_CONFIGURATION[Addr - 0x2D]);
	}
...
void VL53L1XSensor::VL53L1X_start_ranging()
{
	...
	status = this->write_byte(SYSTEM__MODE_START, 0x40); /* Enable VL53L1X */
	return status;
}
...
void VL53L1XSensor::VL53L1X_stop_ranging()
{
	...
	status = this->write_byte(SYSTEM__MODE_ST, 0x00); /* Disable VL53L1X */
	return status;
}
