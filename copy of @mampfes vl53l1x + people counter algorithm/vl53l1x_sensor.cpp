...
void VL53L1XSensor::setup() {
  ...
  //
  // optical center of each ROI.
  unsigned int opticalCenter[] = {this->x_optical_center_, this->y_optical_center_};  
  
  // An int for switching the zones, left (0) or right(1):
  int zone = 0;
  
  // width and height of the ROI
  int roiHeight = this->roi_height_;
  int roiWidth = this->roi_width_;
  
  // set the ROI 
  // setROI(roiWidth, roiHeight,  opticalCenter[zone]);
...
void VL53L1XSensor::loop() {
  ...
  // As well as of publishing the distance in meters also publish the a mount of people to a entity publish_state(peoplecounter)
  }
}
